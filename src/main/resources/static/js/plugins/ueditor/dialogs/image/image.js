                ...
                try {
                    ...
                    *  var responseText = (ret._raw || ret),
                    ...
                    if (json.state == 'SUCCESS') {
                     ...
                     if (json.code == '200') {
                    	 //获取地址
                    	  ajax.request(FILE_URL+'/getById', {
                              'timeout': 100000,
                              'dataType': 'json',
                          	   data:{id:json.data,token:FILE_MIDDLE_TOKEN},
                              'method': 'get',
                              'onsuccess': function (r) {
                                  try {
                                	    var respData = json = utils.str2json(r.responseText);
                                	    var attach = respData.data.child[0];
            							if(attach!=null){
            								   $file.append('<span class="success"></span>');
            								   var json = attach;
            								   attach.title=attach.source;
                                       	      _this.imageList.push(attach);
            							}else{
